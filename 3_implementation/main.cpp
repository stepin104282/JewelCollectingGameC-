...
int main()
{
	...
	int positionX[4]={0}; // Jewel X Position
	int positionY[4]={0}; // Jewel Y Position
	int randomNum,i=0;
	int b=4;
	...
	srand(time(0)); // if we don't use srand() then every time we compile and run the program
					// we will be getting same coordinate fro jewels hence it is must to use srand function
	...
	j.drawBoundary(x,y,positionX,positionY,&gameOver); // Function Call

	while(1){
		if(kbhit()){
			ch=getch();
			system("cls");
			j.changeUserLocationOnButtonClick(&x,&y,ch,&noOfMoves,&gameQuit);
			if(gameQuit==0){ // if gameQuit is 1 means user has pressed 'q' key
				j.drawBoundary(x,y,positionX,positionY,&gameOver);
				if(gameOver==4){ // if gameOver value becomes 4 means we have collected all the jewels
					system("cls");
					printf("\n[][][][] Well Done [][][][]\n");
					printf("\nYou Collected All The Jewels!\n");
					printf("\nIt Took You %d Moves!\n",noOfMoves);
					printf("\n[][][][] Game Over [][][][]\n");
				...
				system("cls");
				printf("\nOh No! You Quit!!\n");
				printf("\nYou Collected %d Jewels!\n",gameOver); // gameOver will give the number of jewel collected.
